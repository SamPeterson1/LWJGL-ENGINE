...
public class Game {
	...
	public void init() {
		...
		shader.create();
		renderer = new Renderer(shader, cam, light);
		...
		batch = new ModelBatch(element1, element2, element3);
	...
	public void dispose() {
		...
		basicShader.remove();
		textShader.remove();
		for(GameElement element: batch.getAllElements()) {
			element.getModel().remove();
		}
